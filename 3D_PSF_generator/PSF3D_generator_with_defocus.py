...
# %% figure 1
z_in_focus = round(Npixels/2)
z_out_of_focus = Npixels-1
...
axs[0].imshow(PSF3D[z_in_focus,:,:], extent = [np.amin(x)+dr,np.amax(x),np.amin(y)+dr,np.amax(y)])


zmax = dz * Npixels 
axs[1].set_title(f'PSF(x,y,{zmax})')  
axs[1].set(xlabel = 'x ($\mu$m)')
axs[1].set(ylabel = 'y ($\mu$m)')
axs[1].imshow(PSF3D[z_out_of_focus,:,:], extent = [np.amin(x)+dr,np.amax(x),np.amin(y)+dr,np.amax(y)])
